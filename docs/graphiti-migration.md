...
> 4. **responses.parse structured output 缺陷**：红杉网关的 `responses.parse` 返回的 `output_text` 把 schema 与数据混合（含 `input_schema` 字段），graphiti `json.loads` 后 ValidationError。**解法：改用 `AzureOpenAILLMClient` 替代 `OpenAIClient`**——它对非 reasoning 模型（deepseek-v4-flash）走 `beta.chat.completions.parse`（`/v1/chat/completions` 的 `response_format`），红杉网关完全支持。同时 patch `AzureOpenAILLMClient._handle_structured_response` 补三元组返回（graphiti Azure client 漏了 token 计数的 bug）。
